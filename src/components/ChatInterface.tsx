...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ onTasksCreated, className = '' }) => {
  ...
  const { addTask, tasks, projects, getCurrentSection, currentDate } = useMarkdownStore();
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      try {
        ...
        await SecureStorage.migrateApiKey(API_KEY_STORAGE_KEY);
        const storedKey = await SecureStorage.getItem(API_KEY_STORAGE_KEY);
  ...
  const buildContext = useCallback((): AIContextInput => ({
    currentTasks: tasks,
    projects,
    currentDate,
    dailySection: getCurrentSection(),
  }), [tasks, projects, getCurrentSection, currentDate]);
  ...
  const handleClearApiKey = async () => {
    try {
      ...
      setMessages([{
        id: '1',
        type: 'assistant',
        content: `${INITIAL_GREETING} Please connect your ${API_KEY_LABEL.toLowerCase()} to continue.`,
        timestamp: new Date(),
      }]);
    } catch (error) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (result.type === 'create_tasks' && result.tasks?.length) {
      ...
      const errorMessage: ChatMessage = {
        ...
        type: 'assistant',
        content: `Sorry, I ran into an issue processing that. Please verify your ${API_KEY_LABEL.toLowerCase()} and try again.`,
        timestamp: new Date(),
      };
    ...
    return (
      <div className={`mypa-card p-6 ${className}`}>
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-neutral-600 dark:text-neutral-400">Loading secure storage...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="text-center space-y-4">
          ...
          <div className="space-y-3">
            <input
              ...
              onKeyPress={(event) => event.key === 'Enter' && handleApiKeySubmit()}
            />
            <button
          ...
          <div className="text-xs text-neutral-500 dark:text-neutral-400 space-y-1">
            ...
            {BACKEND_AVAILABLE ? (
              <p>Need help? Contact the MyPA team for your API credentials.</p>
            ) : (
              <p>Get your key at{' '}
      ...
      <div className={`mypa-card flex flex-col h-96 ${className}`}>
        <div className="flex items-center justify-between p-4 border-b border-neutral-200 dark:border-neutral-700">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-neutral-900 dark:text-neutral-100">{ASSISTANT_NAME} Assistant</span>
        ...
        <form onSubmit={handleSubmit} className="p-4 border-t border-neutral-200 dark:border-neutral-700">
          <div className="flex gap-2">
            <input
              ...
              placeholder={`Ask ${ASSISTANT_NAME} to help with your tasks...`}
