...
const Dashboard: React.FC = () => {
  ...
  const { layout, setCustomizing } = useDashboardStore();
  ...
  // Render individual dashboard sections
  const renderSection = (sectionId: string) => {
    switch (sectionId) {
      ...
      case 'due-overdue':
        if (!(currentSection?.schedule || []).some(task =>
      ...
      case 'completed':
        if (!currentSection?.completed.length) return null;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-6">
      <header className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              onClick={() => setCustomizing(!layout.isCustomizing)}
              className={`mypa-button-${layout.isCustomizing ? 'primary' : 'secondary'} text-sm flex items-center gap-2 hover:scale-105 transition-transform duration-200`}
              title="Customize Dashboard"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
              </svg>
              <span className="hidden sm:inline">{layout.isCustomizing ? 'Done' : 'Customize'}</span>
            </button>
            <Link
              to="/settings"
      ...
      {layout.isCustomizing && <DashboardCustomizer />}



      {/* Dynamic Customizable Dashboard Layout */}
      <div className="space-y-6">
        {layout.sections
          .filter(section => section.visible)
          .sort((a, b) => a.position - b.position)
          .map((section) => {
            const sectionContent = renderSection(section.component);
            ...
            return (
              <DraggableSection
                key={section.id}
                section={section}
                className={section.component === 'priorities' ? 'mb-8' : ''}
              >
                {sectionContent}
              </DraggableSection>
            );
